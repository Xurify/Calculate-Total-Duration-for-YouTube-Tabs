...
export async function loadStorage(): Promise<{
  ...
  if (!browser.storage?.local) {
    ...
    return { sortByDuration: false, excludedUrls: [], smartSync: true, thumbnailQuality: 'high', metadataCache: {} };
  }
  const data = await browser.storage.local.get(["sortByDuration", "excludedUrls", "smartSync", "thumbnailQuality", "metadataCache"]);
  return {
